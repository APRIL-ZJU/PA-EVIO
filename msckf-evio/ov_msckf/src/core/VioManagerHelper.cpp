...
bool VioManager::try_to_initialize(const ov_core::CameraData &message) {
  ...
  std::thread thread([&] {
    ...
    if (success) {
      ...
      trackFEATS->get_feature_database()->cleanup_measurements(state->_timestamp);
      trackFEATS->set_num_features(std::floor((double)params.num_pts / std::max((double)params.state_options.num_cameras, 1.0)));

      trackEVENTS->get_feature_database()->cleanup_measurements(state->_timestamp);
      trackEVENTS->set_num_features(std::floor((double)params.num_pts / std::max((double)params.state_options.num_event_cameras, 1.0)));
...
void VioManager::retriangulate_active_tracks(const ov_core::CameraData &message) {
  ...
  if(message.sensor_ids.at(0) >= params.state_options.num_cameras){
    last_obs = trackEVENTS->get_last_obs();
    last_ids = trackEVENTS->get_last_ids();
  }
  
  // auto event_last_obs = trackEVENTS->get_last_obs();
  // auto event_last_ids = trackEVENTS->get_last_ids();

  // last_obs.insert(event_last_obs.begin(), event_last_obs.end());
  // last_ids.insert(event_last_ids.begin(), event_last_ids.end());
...
cv::Mat VioManager::get_historical_viz_image() {
  ...
  if (state == nullptr || (trackFEATS == nullptr && trackEVENTS == nullptr))
    return cv::Mat();
  ...
  cv::Mat img_history;
  if(params.state_options.num_cameras && params.state_options.num_event_cameras == 0){
    trackFEATS->display_history(img_history, 255, 255, 0, 255, 255, 255, highlighted_ids, overlay);
  }else if(params.state_options.num_event_cameras){
    trackEVENTS->display_history(img_history, 255, 255, 0, 255, 255, 255, highlighted_ids, overlay);
  }
...
std::vector<cv::Mat> VioManager::get_historical_viz_images() {
  ...
  // Get the current active tracks
  std::vector<cv::Mat> img_histories;
  if(trackFEATS != nullptr && params.state_options.num_cameras > 0){
    cv::Mat img_history;
    trackFEATS->display_history(img_history, 255, 255, 0, 255, 255, 255, highlighted_ids, overlay);
    if (trackARUCO != nullptr) {
      trackARUCO->display_history(img_history, 0, 255, 255, 255, 255, 255, highlighted_ids, overlay);
    ...
    img_histories.push_back(img_history);
  }
  
  if(trackEVENTS != nullptr && params.state_options.num_event_cameras > 0){
    cv::Mat img_history;
    trackEVENTS->display_history(img_history, 255, 255, 0, 255, 255, 255, highlighted_ids, overlay);
    img_histories.push_back(img_history);
  }
