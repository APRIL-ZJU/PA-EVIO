...
VioManager::VioManager(VioManagerOptions &params_) : thread_init_running(false), thread_init_success(false) {
  ...
  for (int i = 0; i < state->_options.num_cameras + state->_options.num_event_cameras; i++) {
  ...
  //===================================================================================
  //===================================================================================
  // When we use event cameras, we first record the cam ids.
  std::vector<size_t> e_ids;
  for(size_t i = 0; i < size_t(params.state_options.num_event_cameras); ++i){
    e_ids.push_back(i + params.state_options.num_cameras);
  }

  // Let's make a feature extractor
  // NOTE: after we initialize we will increase the total number of feature tracks
  // NOTE: we will split the total number of features over all cameras uniformly
  int init_max_features = std::floor((double)params.init_options.init_max_features / std::max((double)params.state_options.num_cameras, (double)params.state_options.num_event_cameras));
  if (params.use_klt) {
  ...
  if (params.state_options.num_event_cameras && params.use_event_klt) {
    trackEVENTS = std::shared_ptr<TrackBase>(new TrackKLT(state->_cam_intrinsics_cameras, init_max_features,
                                                         state->_options.max_aruco_features, params.use_event_stereo, params.histogram_method,
                                                         params.fast_threshold, params.grid_x, params.grid_y, params.min_px_dist, e_ids));
  } else {
    trackEVENTS = std::shared_ptr<TrackBase>(new TrackDescriptor(
        state->_cam_intrinsics_cameras, init_max_features, state->_options.max_aruco_features, params.use_event_stereo, params.histogram_method,
        params.fast_threshold, params.grid_x, params.grid_y, params.min_px_dist, params.knn_ratio));
  }
  ...
  if (params.state_options.num_event_cameras){
    initializer = std::make_shared<ov_init::InertialInitializer>(params.init_options, trackEVENTS->get_feature_database());
  }else{
...
void VioManager::track_image_and_update(const ov_core::CameraData &message_const) {
  ...
  PRINT_INFO(GREEN "[Frame ID]: Cam %d\n" RESET, message_const.sensor_ids.at(0)); 
  ...
  if(message_const.sensor_ids.at(0) < state->_options.num_cameras){
    // Perform our feature tracking!
    trackFEATS->feed_new_camera(message);
  }else{
    // Perform event corner tracking!
    trackEVENTS->feed_new_camera(message);
  }
  ...
  if (!is_initialized_vio && message_const.sensor_ids.at(0) == (state->_options.num_cameras + state->_options.num_event_cameras - 1)) {
...
void VioManager::do_feature_propagate_update(const ov_core::CameraData &message) {
  ...
  if(params.state_options.num_cameras > 0){
  ...
  // Fetch event feature tracks
  if(params.state_options.num_event_cameras > 0){
    std::vector<std::shared_ptr<Feature>> feats_event_lost, feats_event_marg, feats_event_slam;
    feats_event_lost = trackEVENTS->get_feature_database()->features_not_containing_newer(state->_timestamp, false, true);
    feats_lost.insert(feats_lost.end(), feats_event_lost.begin(), feats_event_lost.end());
    if ((int)state->_clones_IMU.size() > state->_options.max_clone_size || (int)state->_clones_IMU.size() > 5) {
      feats_event_marg = trackEVENTS->get_feature_database()->features_containing(state->margtimestep(), false, true);
      feats_marg.insert(feats_marg.end(),feats_event_marg.begin(),feats_event_marg.end());
    }
  }
  ...
  std::cout << "Cam id: " << message.sensor_ids.at(0) << " lost features: " << feats_lost.size() << std::endl;
  ...
  for (std::pair<const size_t, std::shared_ptr<Landmark>> &landmark : state->_features_SLAM) {
    ...
    bool current_unique_cam = true;
    if (landmark.second->update_fail_count > 1)
      landmark.second->should_marg = true;

    bool is_event_cam = state->_options.num_event_cameras && (message.sensor_ids[0] == (state->_options.num_cameras + state->_options.num_event_cameras - 1));
    ...
    if (feat2 == nullptr && current_unique_cam && !is_event_cam)
      landmark.second->should_marg = true;

    // Process event feature
    std::shared_ptr<Feature> feat3 = trackEVENTS->get_feature_database()->get_feature(landmark.second->_featid);
    if (feat3 != nullptr)
      feats_slam.push_back(feat3);
    if (feat3 == nullptr && current_unique_cam && is_event_cam)
      landmark.second->should_marg = true;
  }
  ...
  // if (message.sensor_ids.at(0) == params.state_options.num_cameras + params.state_options.num_event_cameras - 1) {
  ...
  trackEVENTS->get_feature_database()->cleanup();
  if (trackARUCO != nullptr) {
  ...
  if ((int)state->_clones_IMU.size() > state->_options.max_clone_size) {
    ...
    trackEVENTS->get_feature_database()->cleanup_measurements(state->margtimestep());
    if (trackARUCO != nullptr) {
  ...
  // std::cout << "Cam id: " << message.sensor_ids.at(0) << " Frame is Marginalized at Time: " << std::setprecision(15) << state->margtimestep() << std::endl;
