...
struct VioManagerOptions {
  ...
  void print_and_load_state(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
    if (parser != nullptr) {
      ...
      // Load Event Cams
      for (int i = 0; i < state_options.num_event_cameras; i++) {

        // Time offset (use the first one)
        // TODO: support multiple time offsets between cameras
        if (i == 0) {
          parser->parse_external("relative_config_imuevent", "cam" + std::to_string(i), "timeshift_cam_imu", calib_camimu_dt, false);
        }

        // Distortion model
        std::string dist_model = "radtan";
        parser->parse_external("relative_config_imuevent", "cam" + std::to_string(i), "distortion_model", dist_model);

        // Distortion parameters
        std::vector<double> cam_calib1 = {1, 1, 0, 0};
        std::vector<double> cam_calib2 = {0, 0, 0, 0};
        parser->parse_external("relative_config_imuevent", "cam" + std::to_string(i), "intrinsics", cam_calib1);
        parser->parse_external("relative_config_imuevent", "cam" + std::to_string(i), "distortion_coeffs", cam_calib2);
        Eigen::VectorXd cam_calib = Eigen::VectorXd::Zero(8);
        cam_calib << cam_calib1.at(0), cam_calib1.at(1), cam_calib1.at(2), cam_calib1.at(3), cam_calib2.at(0), cam_calib2.at(1),
            cam_calib2.at(2), cam_calib2.at(3);
        cam_calib(0) /= (downsample_cameras) ? 2.0 : 1.0;
        ...
        // FOV / resolution
        std::vector<int> matrix_wh = {1, 1};
        parser->parse_external("relative_config_imuevent", "cam" + std::to_string(i), "resolution", matrix_wh);
        matrix_wh.at(0) /= (downsample_cameras) ? 2.0 : 1.0;
        matrix_wh.at(1) /= (downsample_cameras) ? 2.0 : 1.0;

        // Extrinsics
        Eigen::Matrix4d T_CtoI = Eigen::Matrix4d::Identity();
        parser->parse_external("relative_config_imuevent", "cam" + std::to_string(i), "T_imu_cam", T_CtoI);
        ...
        // Create intrinsics model
        if (dist_model == "equidistant") {
          camera_intrinsics.insert({i+state_options.num_cameras, std::make_shared<ov_core::CamEqui>(matrix_wh.at(0), matrix_wh.at(1))});
          camera_intrinsics.at(i+state_options.num_cameras)->set_value(cam_calib);
        } else {
          camera_intrinsics.insert({i+state_options.num_cameras, std::make_shared<ov_core::CamRadtan>(matrix_wh.at(0), matrix_wh.at(1))});
          camera_intrinsics.at(i+state_options.num_cameras)->set_value(cam_calib);
        }
        camera_extrinsics.insert({i+state_options.num_cameras, cam_eigen});
      }
    ...
    if ((state_options.num_cameras + state_options.num_event_cameras) != (int)camera_intrinsics.size() || (state_options.num_cameras + state_options.num_event_cameras) != (int)camera_extrinsics.size()) {
      ...
      PRINT_ERROR(RED "[SIM]: got %d but expected %d max cameras (camera_intrinsics)\n" RESET, (int)camera_intrinsics.size(),
                  state_options.num_cameras + state_options.num_event_cameras);
  ...
  /// If we should process two cameras are being stereo or binocular. If binocular, we do monocular feature tracking on each image.
  bool use_event_stereo = false;

  /// If we should use KLT tracking, or descriptor matcher
  bool use_klt = true;

  /// If we should use KLT tracking, or descriptor matcher for event cameras
  bool use_event_klt = true;
  ...
  void print_and_load_trackers(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
    if (parser != nullptr) {
      parser->parse_config("use_event_stereo", use_event_stereo);
      parser->parse_config("use_event_klt", use_event_klt);
