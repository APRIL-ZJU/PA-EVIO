...
public:
  ...
  /// Callback for event cameras information
  void callback_events(const dvs_msgs::EventArray::ConstPtr& msg, int cam_id);
...
protected:
  ...
  std::vector<std::shared_ptr<adaptive_time_surface::AdaptiveTSurface>> _tsg;

  // Our publishers
  image_transport::Publisher it_pub_tracks, it_pub_loop_img_depth, it_pub_loop_img_depth_color;
  std::vector<image_transport::Publisher> vit_pub_tracks;
  ...
  // Temporary camera data queue
  std::deque<ov_core::CameraData> temp_camera_queue;
  std::deque<ov_core::CameraData> temp_event_camera_queue;
