...
ROS1Visualizer::ROS1Visualizer(std::shared_ptr<ros::NodeHandle> nh, std::shared_ptr<VioManager> app, std::shared_ptr<Simulator> sim)
  ...
  // Setup event map generator
  for(int i = 0; i < _app->get_state()->_options.num_event_cameras; i++){
    _tsg.push_back(std::make_shared<adaptive_time_surface::AdaptiveTSurface>(*_nh));
  }
  ...
  int num_pub = (_app->get_state()->_options.num_cameras && _app->get_state()->_options.num_event_cameras) > 0 ? 2:1; 
  std::cout << "pub: " << num_pub << std::endl;
  for(int i = 0; i < num_pub; ++i){
    std::string pub_name = "trackhist_" + std::to_string(i);
    vit_pub_tracks.push_back(it.advertise(pub_name, 2));
  }
...
void ROS1Visualizer::setup_subscribers(std::shared_ptr<ov_core::YamlParser> parser) {
  ...
  if (_app->get_params().state_options.num_cameras == 2) {
    ...
    for (int i = 0; i < _app->get_params().state_options.num_cameras; i++) {
      ...
      // create subscriber
      subs_cam.push_back(_nh->subscribe<sensor_msgs::Image>(cam_topic, 100, boost::bind(&ROS1Visualizer::callback_monocular, this, _1, i)));
      PRINT_INFO("subscribing to cam (mono): %s\n", cam_topic.c_str());
    }

    // Load Event topics
    for (int i = 0; i < _app->get_params().state_options.num_event_cameras; i++) {
      // read in the topic
      std::string cam_topic;
      _nh->param<std::string>("topic_camera" + std::to_string(i), cam_topic, "/cam" + std::to_string(i+_app->get_params().state_options.num_cameras) + "/image_raw");
      parser->parse_external("relative_config_imuevent", "cam" + std::to_string(i), "rostopic", cam_topic);
      // create subscriber
      subs_cam.push_back(_nh->subscribe<dvs_msgs::EventArray>(cam_topic, 100, boost::bind(&ROS1Visualizer::callback_events, this, _1, i+_app->get_params().state_options.num_cameras)));
      PRINT_INFO("subscribing to Event cam (mono): %s\n", cam_topic.c_str());
    }
  }
}
...
void ROS1Visualizer::callback_inertial(const sensor_msgs::Imu::ConstPtr &msg) {
  ...
  std::thread thread([&] {
    ...
    size_t num_unique_cameras = params.state_options.num_cameras + params.state_options.num_event_cameras;
...
void ROS1Visualizer::callback_monocular(const sensor_msgs::ImageConstPtr &msg0, int cam_id0) {
  ...
  if (int(msg0->width) != _app->get_params().camera_intrinsics[cam_id0]->w() || int(msg0->height) != _app->get_params().camera_intrinsics[cam_id0]->h()) {
    PRINT_ERROR("image size does not match the size specified in the yaml file\n");
    return;
  }
  ...
  //Method 2
  // If we use event camera, we just store current camera data into temporary queues
  if(_app->get_state()->_options.num_event_cameras > 0){
    temp_camera_queue.push_back(message);
    std::sort(temp_camera_queue.begin(), temp_camera_queue.end());
  }else{
...
void ROS1Visualizer::callback_events(const dvs_msgs::EventArray::ConstPtr& msg, int cam_id){
  int index = cam_id - _app->get_state()->_options.num_cameras;
  _tsg[index]->feed_new_events(msg);

  // Temporary variables that stores TimeSurface and its timestamp
  ros::Time current_time;
  cv::Mat current_map; 

  std::lock_guard<std::mutex> lck(camera_queue_mtx);
  int map_count = 0;
  std::deque<ov_core::CameraData> temp_map_queue;
  // Method 1: When receiving an event message, get the ATS at the last event timestamp (Event only mode)
  // Get TimeSurface, otherwise quit
  if(_app->get_state()->_options.num_event_cameras > 0 && _app->get_state()->_options.num_cameras == 0){
    current_time = _tsg[index]->get_latest_event_timestamp();

    if(_tsg[index]->get_latest_map(current_map, current_time)){
      ov_core::CameraData message;
      message.timestamp = current_time.toSec();
      message.sensor_ids.push_back(cam_id);
      message.images.push_back(current_map.clone());

      if (_app->get_params().use_mask) {
        message.masks.push_back(_app->get_params().masks.at(cam_id));
      } else {
        message.masks.push_back(cv::Mat::zeros(current_map.rows, current_map.cols, CV_8UC1));
      }

      temp_map_queue.push_back(message);
      map_count++;
    }else{
      return;
    }
  ...
  // Method 2: When receiving an event message, get ATSs with adaptive methods
  while(_tsg[index]->get_next_map(current_map, current_time)){
    ov_core::CameraData message;
    message.timestamp = current_time.toSec();
    message.sensor_ids.push_back(cam_id);
    message.images.push_back(current_map.clone());
  ...
  // If we do not use camera, just push TimeSurface into camera queue
  if(_app->get_params().state_options.num_cameras == 0){
    camera_queue.insert(camera_queue.end(), temp_map_queue.begin(), temp_map_queue.end());
    // camera_queue.push_back(message);
    std::sort(camera_queue.begin(), camera_queue.end());
  }else{
    // append it to our temporary queue of event images
    temp_event_camera_queue.insert(temp_event_camera_queue.end(), temp_map_queue.begin(), temp_map_queue.end());
    // temp_event_camera_queue.push_back(message);
    std::sort(temp_event_camera_queue.begin(), temp_event_camera_queue.end());

    // Get maximum available event timestamp
    double max_event_timestamp = _tsg[0]->get_latest_event_timestamp().toSec();
    for(int i = 1; i < _app->get_state()->_options.num_event_cameras; i++){
      max_event_timestamp = std::min(max_event_timestamp, _tsg[i]->get_latest_event_timestamp().toSec());
    }

    auto it = temp_camera_queue.begin();
    while(it != temp_camera_queue.end()){
      // Remove data that has old timestamp (system state)
      if(it->timestamp < _app->get_state()->_timestamp){
        temp_camera_queue.pop_front();
        it = temp_camera_queue.begin();
        continue;
      }

      // Make sure that the camera data has smaller timestamp than event data
      if(it->timestamp < max_event_timestamp){
        ov_core::CameraData message_event;
        bool bTimeSurface_succ = true;
        // Process every event cam
        for(int i = 0; i < _app->get_state()->_options.num_event_cameras; i++){
          cv::Mat event_map;
          ros::Time event_time = ros::Time((*it).timestamp);
          if(_tsg[i]->get_specific_map(event_map, event_time)){
            int event_id = _app->get_state()->_options.num_cameras + i;
            message_event.timestamp = it->timestamp;
            message_event.sensor_ids.push_back(event_id);
            message_event.images.push_back(event_map.clone());

            if (_app->get_params().use_mask) {
              message_event.masks.push_back(_app->get_params().masks.at(event_id));
            }else{
              message_event.masks.push_back(cv::Mat::zeros(event_map.rows, event_map.cols, CV_8UC1));
            }

          }else{
            bTimeSurface_succ = false;
            break;
          }
        }

        if(bTimeSurface_succ){
          // append it to our queue of images
          camera_queue.push_back(*it);
          camera_queue.push_back(message_event);
        }else{
          // If we cannot get TimeSurface, just push the camera message
          camera_queue.push_back(*it);
        }

        // Push back Time Surface that has smaller timestamp than the camera message
        for(auto it_temp = temp_event_camera_queue.begin(); it_temp != temp_event_camera_queue.end();){
          // Remove data that has old timestamp (system state)
          if(it_temp->timestamp < _app->get_state()->_timestamp){
            it_temp = temp_event_camera_queue.erase(it_temp);
            continue;
          }

          // Remove data that has old timestamp (camera queue)
          // if(!camera_queue.empty() && it_temp->timestamp < camera_queue.at(0).timestamp){
          //   it_temp = temp_event_camera_queue.erase(it_temp);
          //   continue;
          // }

          if(it_temp->timestamp < message_event.timestamp){
            camera_queue.push_back(*it_temp);
            it_temp = temp_event_camera_queue.erase(it_temp);
          }else{
            ++it_temp;
          }
...
void ROS1Visualizer::publish_images() {
  ...
  // // Get our image of history tracks
  // std::vector<cv::Mat> img_histories = _app->get_historical_viz_images();
  // if (img_histories.empty())
  //   return;

  // for(size_t i = 0; i < vit_pub_tracks.size(); ++i){ 
  //   // Create our message
  //   std_msgs::Header header;
  //   header.stamp = ros::Time::now();
  //   header.frame_id = "cam_" + std::to_string(i);
  //   sensor_msgs::ImagePtr exl_msg = cv_bridge::CvImage(header, "bgr8", img_histories[i]).toImageMsg();

  //   // Publish
  //   vit_pub_tracks[i].publish(exl_msg);
  // }
}
