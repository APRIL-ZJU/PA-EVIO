...
struct InertialInitializerOptions {
  ...
  void print_and_load_state(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
    if (parser != nullptr) {
      ...
      parser->parse_config("max_event_cameras", num_event_cameras); // might be redundant
      ...
      for (int i = 0; i < num_event_cameras; i++) {
        ...
        // Create intrinsics model
        if (dist_model == "equidistant") {
          camera_intrinsics.insert({i+num_cameras, std::make_shared<ov_core::CamEqui>(matrix_wh.at(0), matrix_wh.at(1))});
          camera_intrinsics.at(i+num_cameras)->set_value(cam_calib);
        } else {
          camera_intrinsics.insert({i+num_cameras, std::make_shared<ov_core::CamRadtan>(matrix_wh.at(0), matrix_wh.at(1))});
          camera_intrinsics.at(i+num_cameras)->set_value(cam_calib);
        }
        camera_extrinsics.insert({i+num_cameras, cam_eigen});
      }
    }
    ...
    if (num_cameras + num_event_cameras != (int)camera_intrinsics.size() || num_cameras + num_event_cameras!= (int)camera_extrinsics.size()) {
