...
public:
  ...
  explicit TrackKLT(std::unordered_map<size_t, std::shared_ptr<CamBase>> cameras, int numfeats, int numaruco, bool stereo,
                    HistogramMethod histmethod, int fast_threshold, int gridx, int gridy, int minpxdist, std::vector<size_t> eids = std::vector<size_t>())
      : TrackBase(cameras, numfeats, numaruco, stereo, histmethod), threshold(fast_threshold), grid_x(gridx), grid_y(gridy),
        min_px_dist(minpxdist), e_ids(eids) {}
...
protected:
   ...
   */
  void perform_detection_monocular(const std::vector<cv::Mat> &img0pyr, const cv::Mat &mask0, size_t cam_id, std::vector<cv::KeyPoint> &pts0,
   ...
   * If the second vector is non-empty, it will be used as an initial guess of where the keypoints are in the second image.
   */
  void perform_matching_for_event_cameras(const std::vector<cv::Mat> &img0pyr, const std::vector<cv::Mat> &img1pyr, std::vector<cv::KeyPoint> &kpts0,
  ...
  int pyr_levels = 3; // 5
  cv::Size win_size = cv::Size(24, 24); // (15, 15)
  ...
  // Indicate which id belongs to event
  std::vector<size_t> e_ids;
  int ts_type; // 0 for uninitialized, 1 for ts, 2 for polarity-weighted ts
};
