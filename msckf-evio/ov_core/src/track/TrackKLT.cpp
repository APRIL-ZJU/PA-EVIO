...
void TrackKLT::feed_monocular(const CameraData &message, size_t msg_id) {
  ...
  bool isEventCamera = std::find(e_ids.begin(), e_ids.end(), cam_id) != e_ids.end();
  ...
  if (pts_last[cam_id].empty()) {
    ...
    perform_detection_monocular(imgpyr, mask, cam_id, good_left, good_ids_left);
  ...
  perform_detection_monocular(img_pyramid_last[cam_id], img_mask_last[cam_id], cam_id, pts_left_old, ids_left_old);
  ...
  if(!isEventCamera){
    ...
    perform_matching_for_event_cameras(img_pyramid_last[cam_id], imgpyr, pts_left_old, pts_left_new, cam_id, cam_id, mask_ll);
  }
...
void TrackKLT::perform_detection_monocular(const std::vector<cv::Mat> &img0pyr, const cv::Mat &mask0, size_t cam_id, std::vector<cv::KeyPoint> &pts0,
  ...
  bool isEventCamera = std::find(e_ids.begin(), e_ids.end(), cam_id) != e_ids.end();
  if(isEventCamera && (ts_type == 2)){
    cv::Mat image_np = img0pyr.at(0);
    image_np.convertTo(image_np, CV_64F);
    image_np = 2 * (image_np / 255) - 1;
    image_np = cv::abs(image_np) * 255;
    image_np.convertTo(image_np, CV_8U);

    Grider_GRID::perform_griding(image_np, mask0_updated, valid_locs, pts0_ext, num_features, grid_x, grid_y, threshold, true);
  }else{
  ...
  for (size_t i = 0; i < pts0_new.size(); i++) {
    ...
    size_t temp = num_cams * (++currid) + cam_id;
    ids0.push_back(temp);
  }
}
...
void TrackKLT::perform_matching_for_event_cameras(const std::vector<cv::Mat> &img0pyr, const std::vector<cv::Mat> &img1pyr, std::vector<cv::KeyPoint> &kpts0,
                                std::vector<cv::KeyPoint> &kpts1, size_t id0, size_t id1, std::vector<uchar> &mask_out) {
  ...
  // Convert keypoints into points (stupid opencv stuff)
  std::vector<cv::Point2f> pts0, pts1, pts1_inv, pts1_np;
  for (size_t i = 0; i < kpts0.size(); i++) {
    pts0.push_back(kpts0.at(i).pt);
    pts1.push_back(kpts1.at(i).pt);
    pts1_inv.push_back(kpts1.at(i).pt);
    pts1_np.push_back(kpts1.at(i).pt);
  }
  ...
  bool event_flag = !e_ids.empty() && std::find(e_ids.begin(), e_ids.end(), id0) != e_ids.end() && std::find(e_ids.begin(), e_ids.end(), id1) != e_ids.end();

  if(ts_type == 0){
    cv::Mat zeroPixels;
    cv::compare(img_curr[id1], 0, zeroPixels, cv::CMP_EQ);
    int zero_count = cv::countNonZero(zeroPixels);

    cv::Mat midPixels;
    cv::compare(img_curr[id1], 128, midPixels, cv::CMP_EQ);
    int mid_count = cv::countNonZero(midPixels);

    if(mid_count > zero_count){
      ts_type = 2; // means it is a time surface with polarity
    }else{
      ts_type = 1; // means it is a time surface without polarity
    }
  }

  bool polarity_flag = (ts_type == 2);
  // Check the ids of the event cameras
  // event_flag = false;
  if(event_flag && polarity_flag){
    rT_compensation_begin = boost::posix_time::microsec_clock::local_time();

    // Compensation using an inversed polarity weighted ATS
    cv::Mat image_raw = img_curr[id1];
    cv::Mat image_inv;
    cv::bitwise_not(image_raw, image_inv);

    std::vector<cv::Mat> img1pyr_inv;
    cv::buildOpticalFlowPyramid(image_inv, img1pyr_inv, win_size, pyr_levels);

    std::vector<uchar> mask_klt_inv;
    std::vector<float> error_inv;

    cv::calcOpticalFlowPyrLK(img0pyr, img1pyr_inv, pts0, pts1_inv, mask_klt_inv, error_inv, win_size, pyr_levels, term_crit, cv::OPTFLOW_USE_INITIAL_FLOW);

    // Normalize these points, so we can then do ransac
    // We don't want to do ransac on distorted image uvs since the mapping is nonlinear
    std::vector<cv::Point2f> pts1_inv_n;
    for (size_t i = 0; i < pts0.size(); i++) {
      pts1_inv_n.push_back(camera_calib.at(id1)->undistort_cv(pts1_inv.at(i)));
    }

    // // Method 1: merge result
    int count1 = 0, count2 = 0;
    // // for(size_t i = 0; i < pts0.size(); i++) {
    // //   if(mask_klt2[i] == 1 && (error2[1] < error[i])) {
    // //     pts1_n[i] = pts1_inverse_n[i];
    // //     mask_klt[i] = mask_klt2[i];
    // //     error[i] = error2[i];
    // //     count++;
    // //   }
    // // }
    // // PRINT_INFO(RED "[WARN]: KLT: Num of new points %d\n" RESET, count);

    // // Method 2: better result
    // // int num_1 = 0, num_2 = 0;
    // // for(size_t i = 0; i < pts0.size(); i++) {
    // //   num_1 += mask_klt[i];
    // //   num_2 += mask_klt2[i];
    ...
    // // Method 3: best result
    int num_1 = 0, num_2 = 0;
    for(size_t i = 0; i < pts0.size(); i++) {
      num_1 += mask_klt[i];
      num_2 += mask_klt_inv[i];
    }
    if(num_1 < num_2) {
      for(size_t i = 0; i < pts0.size(); i++) {
        if(mask_klt_inv[i] == 1 && (error_inv[i] < error[i])) {
          pts1.at(i) = pts1_inv.at(i);
          pts1_n[i] = pts1_inv_n[i];
          mask_klt[i] = mask_klt_inv[i];
          error[i] = error_inv[i];
          count1++;
        }
      }
    ...
    // Compensation when using polarity-weighted ATS
    cv::Mat image_last_np = img_last[id0];
    image_last_np.convertTo(image_last_np, CV_64F);
    image_last_np = 2 * (image_last_np / 255) - 1;
    image_last_np = cv::abs(image_last_np) * 255;
    image_last_np.convertTo(image_last_np, CV_8U);

    cv::Mat image_curr_np = img_curr[id1];
    image_curr_np.convertTo(image_curr_np, CV_64F);
    image_curr_np = 2 * (image_curr_np / 255) - 1;
    image_curr_np = cv::abs(image_curr_np) * 255;
    image_curr_np.convertTo(image_curr_np, CV_8U);

    std::vector<cv::Mat> img0pyr_np, img1pyr_np;
    cv::buildOpticalFlowPyramid(image_last_np, img0pyr_np, win_size, pyr_levels);
    cv::buildOpticalFlowPyramid(image_curr_np, img1pyr_np, win_size, pyr_levels);

    std::vector<uchar> mask_klt_np;
    std::vector<float> error_np;

    cv::calcOpticalFlowPyrLK(img0pyr_np, img1pyr_np, pts0, pts1_np, mask_klt_np, error_np, win_size, pyr_levels, term_crit, cv::OPTFLOW_USE_INITIAL_FLOW);

    std::vector<cv::Point2f> pts1_np_n;
    for (size_t i = 0; i < pts0.size(); i++) {
      pts1_np_n.push_back(camera_calib.at(id1)->undistort_cv(pts1_np.at(i)));
    }

    // int count = 0;
    for(size_t i = 0; i < pts0.size(); i++) {
      if(mask_klt_np[i] == 1 && (error_np[i] < error[i])) {
        pts1.at(i) = pts1_np.at(i);
        pts1_n[i] = pts1_np_n[i];
        mask_klt[i] = mask_klt_np[i];
        error[i] = error_np[i];
        count2++;
      }
    }
