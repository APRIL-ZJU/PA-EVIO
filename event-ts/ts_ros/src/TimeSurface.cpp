...
void TimeSurface::createTimeSurfaceAtTime(const ros::Time& external_sync_time)
{
  ...
  if (time_surface_mode_ == BACKWARD && time_surface_pub_.getNumSubscribers() > 0)
  {
...
void TimeSurface::eventsCallback(const dvs_msgs::EventArray::ConstPtr& msg)
{
  ...
  const dvs_msgs::Event& last_event = events_.back();
  ros::Time last_event_time = last_event.ts;

  boost::posix_time::ptime rt1 = boost::posix_time::microsec_clock::local_time();
  // createTimeSurfaceAtTime(last_event_time);
  createTimeSurfaceAtTime_hyperthread(last_event_time);
  boost::posix_time::ptime rt2 = boost::posix_time::microsec_clock::local_time();
  double total_time = (rt2 - rt1).total_microseconds() * 1e-6;

  std::ofstream outfile;
  outfile.open("/media/zju/SSD/evins_results/vicon/ts_generation_time.txt", std::ios::app);
  outfile << std::fixed << std::setprecision(10) << total_time << std::endl;
  outfile.close();
